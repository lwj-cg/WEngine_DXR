...
struct GlassMaterial
{
    ...
    float3 f(float3 woWorld, float3 wiWorld, BxDFType flags, Onb onb, float3 ng)
    {
        ...
        if (reflect)
        {
            f = microRefl.Pdf(wo, wi);
        }
        else
        {
            f = microTrans.Pdf(wo, wi);
        }
        return f;
    }
    ...
    float3 Sample_f(float3 woWorld, out float3 wiWorld, float2 u, out float pdf, BxDFType type, inout BxDFType sampledType, Onb onb)
    {
        int matchingComps = 2;
...
GlassMaterial createGlassMaterial(float3 R, float3 T, float urough, float vrough, float eta)
{
    ...
    FresnelDielectric fresnel = createFresnelDielectric(1.f, eta);
                      ...
                      bool specular, Onb onb, float scene_epsilon)
{
    ...
    if(lightPdf>0&&!isBlack(Li))
    {
        ...
        float3 f;
        f = mat.f(it.wo, wi, bsdfFlags, onb, it.ng);
